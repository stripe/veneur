...
func NewFromConfig(logger *logrus.Logger, conf Config) (*Server, error) {
	...
	if conf.SignalfxAPIKey != "" {
		...
		sfxSink, err := signalfx.NewSignalFxSink(conf.SignalfxHostnameTag, conf.Hostname, ret.TagsAsMap, log, fallback, conf.SignalfxVaryKeyBy, byTagClients, conf.SignalfxFlushMaxPerBody)
		if err != nil {
			return ret, err
		}
