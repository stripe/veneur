...
var testCases = []struct {
	...
	{
		metrics(
			mixedhisto("test", 1, hn("a")),
			mixedhisto("test", 2, hn("b")),
			mixedhisto("test", 3, hn("c")),
		),
		samplers.TMetrics(
	...
	{
		metrics(statusCheck("test", 1, "message")),
		samplers.TMetrics(samplers.TStatus("test", 1, "message")),
		"basic status check failed",
	},
	{
		metrics(
			statusCheck("test", 0, "message"),
			statusCheck("test2", 1, "message2"),
		),
		samplers.TMetrics(
			samplers.TStatus("test", 0, "message"),
			samplers.TStatus("test2", 1, "message2"),
		),
		"multiple status checks failed",
	},
	{
		metrics(
			statusCheck("test", 1, "message"),
			statusCheck("test", 2, "message2"),
		),
		samplers.TMetrics(samplers.TStatus("test", 2, "message2")),
		"status check not last write wins",
	},
	{
		metrics(statusCheck("test", 0, "message", hn("myhost"))),
		samplers.TMetrics(samplers.TStatus("test", 0, "message", samplers.OptHostname("myhost"))),
		"status check incorrect host name",
	},
	{
		metrics(
			statusCheck("test", 1, "message", tags("a:b")),
			statusCheck("test", 2, "message", tags("c:d")),
		),
		samplers.TMetrics(
			samplers.TStatus("test", 1, "message", samplers.OptTags("a:b")),
			samplers.TStatus("test", 2, "message", samplers.OptTags("c:d")),
		),
	...
	},
	{
		...
		samplers.TMetrics(
			samplers.TStatus("test", 1, "message"),
			samplers.TCounter("test", 3),
		),
		"status check not aggregated separately by type",
	},
}
