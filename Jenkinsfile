pipeline {
    ...
    parameters {
        ...
        string(name: "IMAGE_TAG", defaultValue: "v14.2.0-1", description: "Image tag")
            ...
            steps {
                script {
                    dir('public-docker-images') {
                        sh "docker build -f Dockerfile-debian-sid --build-arg VERSION=${env.GIT_COMMIT} --build-arg BUILD_REF=${params.BRANCH_REV} -t ${env.IMAGE_NAME} --no-cache ."
                        sh "docker build -f Dockerfile-alpine --build-arg VERSION=${env.GIT_COMMIT} --build-arg BUILD_REF=${params.BRANCH_REV} -t ${env.IMAGE_NAME_ALPINE} --no-cache ."
                    }
                }
            }
        }
