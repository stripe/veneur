...
func TestNewSignalFxSink(t *testing.T) {
	...
	sink, err := NewSignalFxSink("host", "glooblestoots", map[string]string{"yay": "pie"}, logrus.New(), client, "", nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	err = sink.Start(nil)
...
func TestSignalFxFlushRouting(t *testing.T) {
	...
	sink, err := NewSignalFxSink("host", "glooblestoots", map[string]string{"yay": "pie"}, logrus.New(), fakeSink, "", nil, nil, nil)
...
func TestSignalFxFlushWithDrops(t *testing.T) {
	fakeSink := NewFakeSink()
	sink, err := NewSignalFxSink("host", "glooblestoots", map[string]string{"yay": "pie"}, logrus.New(), fakeSink, "", nil, []string{"foo.bar"}, []string{"baz:gorch"})
	assert.NoError(t, err)
...
func TestSignalFxSetExcludeTags(t *testing.T) {
	fakeSink := NewFakeSink()
	sink, err := NewSignalFxSink("host", "glooblestoots", map[string]string{"yay": "pie", "boo": "snakes"}, logrus.New(), fakeSink, "", nil, nil, nil)
...
func TestSignalFxFlushMultiKey(t *testing.T) {
	...
	sink, err := NewSignalFxSink("host", "glooblestoots", map[string]string{"yay": "pie"}, logrus.New(), fallback, "test_by", map[string]DPClient{"available": specialized}, nil, nil)
